#Peto
plot_Peto <- function(n, lambdaT, lambdaC, alpha, show, table, value, new_plot, col){
  graycol = 'gray'
  p  = seq(0,1,by = 0.001)
  ...
  for (i in 2:nrow(outcome)){
    if (!is.na(outcome[i,n+2])){
      ...
      v <- (outcome[i,n+2]^2) * (1-outcome[i,n+2])/(n-outcome[i,1]) #V[S(t)] Peto
      # v <- (outcome[i,n+2]^2) * (1-outcome[i,n+2])/(outcome[i,1])
  ...
  for (j in 1:(length(lim) - 1)){
    ...
    lines(p[sum(lim[j] > p):sum(lim[j+1] > p)], poly[sum(lim[j] > p):sum(lim[j+1] > p)],
          col = col)
  }
  if(table == TRUE){
    p  <- value
