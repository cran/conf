# Exp-Greenwood
plot_ExpGreenwood <- function(n, lambdaT, lambdaC, alpha, show, table, value, new_plot, col){
  graycol = 'gray'
  p  = seq(0,1,by = 0.001)
  ...
  for (i in 2:nrow(outcome)){
    if (!is.na(outcome[i,n+2])){
      ...
      v <- bigs/(log10(outcome[i,n+2]))^2 # V[S(t)] Exponential Greenwood
      cplus <- log10(-log10(outcome[i,n+2]))+z*sqrt(v)
      cminus <- log10(-log10(outcome[i,n+2]))-z*sqrt(v)
      outcome[i,n+5] <- max(c(exp(-exp(cplus)),0)) # Lower bound
      outcome[i,n+6] <- min(c(exp(-exp(cminus)),1)) # Upper bound
    }
  }
